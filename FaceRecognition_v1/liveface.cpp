...
void liveface::LoadModel(std::vector<ModelConfig>& configs) {
    configs_ = configs;
    //model_num_ = static_cast<int>(configs_.size());
    model_num_ = 1;
    for (int i = 0; i < model_num_; ++i) {
        ncnn::Net* net = new ncnn::Net();
        //std::string project_path = "E:/FaceRecognition/LiveFaceReco_RaspberryPi-master/LiveFaceReco_RaspberryPi-master";
        ...
        std::string param = "models/" + configs_[i].name + ".param";
        std::string model = "models/" + configs_[i].name + ".bin";
...
float liveface::Detect(cv::Mat& src, LiveFaceBox& box)
{
...
cv::Rect liveface::CalculateBox(LiveFaceBox& box, int w, int h, ModelConfig& config) {
    ...
    //float temp = std::min((w - 1) / (float)box_width, (h - 1) / (float)box_height);
    float temp = (w - 1) / (float)box_width < (h - 1) / (float)box_height ? (w - 1) / (float)box_width < (h - 1) : (h - 1) / (float)box_height;
    //float scale = std::min(config.scale, temp);
    float scale = config.scale < temp ? config.scale : temp;
